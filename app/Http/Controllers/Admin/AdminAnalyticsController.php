...
class AdminAnalyticsController extends Controller
{
    public function __invoke(Request $request): View
    {
        ...
        $visitorLocations = (clone $base)->where('event', 'page_view')
            ->where(function ($query) {
                $query->whereNotNull('country')
                    ->orWhereNotNull('region')
                    ->orWhereNotNull('city');
            })
            ->selectRaw("COALESCE(NULLIF(country, ''), 'Unknown country') as country_name, COALESCE(NULLIF(region, ''), 'Unknown region') as region_name, COALESCE(NULLIF(city, ''), 'Unknown city') as city_name, COUNT(DISTINCT visitor_id) as visitors, COUNT(*) as views")
            ->groupBy('country_name', 'region_name', 'city_name')
            ->orderByDesc('visitors')
            ->get();
        $pagesByLocation = (clone $base)->where('event', 'page_view')
            ->where(function ($query) {
                $query->whereNotNull('country')
                    ->orWhereNotNull('region')
                    ->orWhereNotNull('city');
            })
            ->selectRaw("COALESCE(NULLIF(country, ''), 'Unknown country') as country_name, COALESCE(NULLIF(region, ''), 'Unknown region') as region_name, COALESCE(NULLIF(city, ''), 'Unknown city') as city_name, COALESCE(NULLIF(path, ''), '/') as page_path, COUNT(*) as views, MAX(created_at) as last_viewed_at")
            ->groupBy('country_name', 'region_name', 'city_name', 'page_path')
            ->orderByDesc('views')
            ->get()
            ->groupBy(fn ($page) => $page->country_name."\0".$page->region_name."\0".$page->city_name);

        $visitorLocations->each(function ($location) use ($pagesByLocation) {
            $location->pages = $pagesByLocation->get($location->country_name."\0".$location->region_name."\0".$location->city_name, collect());
        });
