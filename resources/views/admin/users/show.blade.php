...
<section class="panel" style="margin-bottom:16px">
    ...
    <div class="trade-summary">
        ...
        <div class="trade-summary-card fees"><span>Total trading fees</span>@forelse($tradeTotals as $total)<strong>{{ $total->currency }} {{ number_format((float) $total->total_fees, 4) }}</strong>@empty<strong>—</strong>@endforelse</div>
    </div>
    ...
    <div class="table-wrap"><table><thead><tr><th>Date</th><th>Market</th><th>Broker</th><th>Side</th><th>Entry / exit</th><th>Quantity</th><th>Trading fees</th><th>Net P&amp;L</th><th>Source</th></tr></thead><tbody>
        @forelse($trades as $trade)
            <tr><td><strong>{{ $trade->date?->format('d M Y') }}</strong><br><span class="muted">{{ $trade->time ? \Illuminate\Support\Carbon::parse($trade->time)->format('h:i A') : '—' }}</span></td><td><strong>{{ $trade->pair }}</strong><br><span class="muted">{{ $trade->strategy ?: ($trade->status ?: '—') }}</span></td><td>{{ $trade->broker ?: ($trade->exchange ?: 'Manual') }}</td><td><span class="badge">{{ $trade->trade_type ?: '—' }}</span></td><td>{{ is_null($trade->entry_price) ? '—' : number_format((float) $trade->entry_price, 4) }} / {{ is_null($trade->exit_price) ? '—' : number_format((float) $trade->exit_price, 4) }}</td><td>{{ $trade->quantity ?? $trade->lot_size ?? '—' }}</td><td>{{ is_null($trade->trading_fees) ? '—' : (($trade->currency ?: $user->currency).' '.number_format((float) $trade->trading_fees, 4)) }}</td><td style="color:{{ $trade->net_pnl >= 0 ? '#86efac' : '#fda4af' }}">{{ $trade->currency ?: $user->currency }} {{ number_format($trade->net_pnl, 2) }}</td><td>{{ $trade->imported_at ? 'Imported' : 'Manual' }}</td></tr>
        @empty<tr><td class="empty" colspan="9">No trades match this view.</td></tr>@endforelse
    </tbody></table></div>
